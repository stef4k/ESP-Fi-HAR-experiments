...
def make_loeo_subject_disjoint_folds(
    ...
    for target_scene in scenes_to_run:
        target_scene_df = manifest[manifest["scene"] == target_scene]
        source_scene_df = manifest[
            (manifest["scene"].isin(scene_set)) & (manifest["scene"] != target_scene)
        ]
        if target_scene_df.empty:
            print(f"[warn] target scene={target_scene} missing in manifest, skipping.")
            continue
        if source_scene_df.empty:
            print(f"[warn] no source-scene data for target scene={target_scene}, skipping.")
            continue

        target_scene_name = str(target_scene_df["scene_name"].iloc[0])
        subjects = sorted(target_scene_df["subject"].unique().tolist())
        if max_subject_folds is not None:
            subjects = subjects[:max_subject_folds]

        for held_out_subject in subjects:
            train_df = source_scene_df[source_scene_df["subject"] != held_out_subject]
            test_df = target_scene_df[target_scene_df["subject"] == held_out_subject]
            ...
            folds.append(
                {
                    ...
                    "fold_id": f"scene_{target_scene}_subject_{held_out_subject}",
                    "train": train_df,
                    "test": test_df,
                }
            )
...
def run_benchmark(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    print("CV scheme: LOEO + subject-disjoint")
    ...
    folds = make_loeo_subject_disjoint_folds(
        manifest,
        scenes_to_run=scenes_to_run,
        max_subject_folds=args.max_subject_folds,
    )
    print("Total LOEO + subject-disjoint folds:", len(folds))
    if not folds:
...
def save_outputs(results_df: pd.DataFrame, summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    raw_path = out_dir / "loeo_subject_disjoint_fold_results.csv"
    summary_path = out_dir / "loeo_subject_disjoint_scene_summary.csv"
    summary_md_path = out_dir / "loeo_subject_disjoint_scene_summary.md"
    summary_txt_path = out_dir / "loeo_subject_disjoint_scene_summary.txt"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    default_out_dir = this_dir / "benchmark_outputs_loeo_subject_disjoint"

    parser = argparse.ArgumentParser(
        description="Run cross-environment LOEO + subject-disjoint benchmark on MAT or RF CSV datasets."
    )
...
def main() -> None:
    ...
    print("\nLOEO + subject-disjoint summary (rounded):")
